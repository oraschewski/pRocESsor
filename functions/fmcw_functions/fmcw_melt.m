...
%   set(gcf,'pos',[1360 554 560 420])
    ...
    if cfg.SaveFig
        %fit_fig = gcf;
        %exportgraphics(fit_fig,[filename1(end-12:end-4) '_StrainFitting.png'],'Resolution',300)
    end
end
...
if cfg.doMeltEstimate
    ...
    if cfg.doPlotMelt || cfg.doPlotAll
        ...
        ax(1) = subplottight(3,1,1);
        fah = plot(f.rangeCoarse(ploti),20*log10(abs(f.specRaw(ploti))),'r'); % profile
        hold on
        gah = plot(g.rangeCoarse(ploti),20*log10(abs(g.specRaw(ploti))),'b'); % profile
        fabedh = plot(f.range(f.bn),20*log10(abs(f.specRaw(f.bn))),'ro'); % bed point (using total range)
        gabedh = plot(g.range(g.bn),20*log10(abs(g.specRaw(g.bn))),'bo'); % bed point (using total range)
